...
The Kustom callback above is invoked by Kustom through a URL we register, so it's protected by an HMAC `?token=`. For fetching the **same** AI recommendations directly — e.g. while onboarding/testing the upsell experience outside the checkout iframe — Hoodtopia also exposes a standalone REST endpoint gated by a **Bearer token**. Both share `src/lib/kustom/upsell.ts`, so the recommendations are identical.

`POST /api/upsell` accepts the bearer token in **either** of two shapes:

- **Mode A — static key (simplest, for partner/portal onboarding):** send `Authorization: Bearer <UPSELL_API_KEY>` directly. One secret, one call, no handshake. Rotate the key to revoke. This is what to hand a partner team.
- **Mode B — handshake JWT (adds expiry + replay protection):** exchange the key for a short-lived token first, then call with that token.

Only `UPSELL_API_KEY` is required for Mode A. `UPSELL_API_JWT_SECRET` (server-only HS256 signing key, ≥32 chars) is only needed if you also want Mode B's handshake. When neither secret is set, `/api/upsell` returns `401`; `/api/upsell/auth` returns `503 not_configured` unless **both** are set.

```jsonc
// Mode A — static key, one call:
POST /api/upsell
Authorization: Bearer <UPSELL_API_KEY>
...
// 2. POST /api/upsell  Authorization: Bearer <token>   (same body/response as Mode A)
```

```bash
# Mode A — hand a partner just UPSELL_API_KEY + BASE:
export UPSELL_API_KEY='<upk_...>'
export BASE='https://hoodtopia.co'
curl -s "$BASE/api/upsell" \
  -H "authorization: Bearer $UPSELL_API_KEY" -H 'content-type: application/json' \
  -d '{"purchase_currency":"GBP","order_lines":[{"type":"physical","reference":"<sku>","name":"x","quantity":1,"unit_price":4500,"tax_rate":2000,"total_amount":4500,"total_tax_amount":750}]}' | jq

# Mode B — derive the digest + drive the handshake flow:
NONCE=$(openssl rand -hex 16)
DIGEST=$(printf '%s' "${NONCE}${UPSELL_API_KEY}" | shasum -a 256 | cut -d' ' -f1)
TOKEN=$(curl -s "$BASE/api/upsell/auth" \
